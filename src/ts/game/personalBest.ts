...
// localStorage, ranked by score. The chart's content hash is stored alongside the
// record so an edit to the chart invalidates a stale best (a different note set)
// instead of crediting an old score to the new notes.
//
...
// Record a run if it beats (by score) the current valid best, replacing any stale
// record. Returns the best that stood *before* this run (null if none) and whether
// this run set a new record, so the results screen can show both.
export function commitPersonalBest(
  ...
  const isRecord = previous === null || candidate.score > previous.score;
  if (isRecord) {
    try {
