...
- **New combo = phrase break.** An object with osu's new-combo flag emits a `break` line before it when **either** the object or the immediately preceding note is `flow`. The following chart row gets `newCombo: true` at compile time. That flag ends the previous flow phrase (consecutive flow anchors link only while the later one has no `break` before it) and records whether a neighbouring non-flow object still shares the combo — when it does, the first/last flow anchor of the phrase uses that neighbour's position for **auto tangent heading only** (no ribbon is drawn to it). A non-flow note also ends a phrase by kind; inert `end` markers do not.
