...
All note types gradually appear as the song approaches their hit time. Cut and flow notes start as faint outlines and fill with color as the hit time approaches. Every note gives a brief size swell that peaks right at its hit time, a subtle visual cue for the beat. Lyric notes show a soft filled containment disc (contrasting with cut's hollow slash) plus a faint outer ghost of the judgement zone; during the approach each mapped character detaches from its storyboard glyph (even before the phrase line fades in, launching from where it will sit) and flies onto the note, crossfading into the canvas-filled glyph as it lands (multi-character notes funnel their characters in sequence). While a lyric is being held, a warm radial halo grows with the sustain — brighter teal while the cursor is inside, dimmer white when the hold lapses without it — and tightens slightly near release so the player senses the hold ending.
