...
Lyric notes display the Japanese characters sung during the hold: charting picks out the most important syllable(s) per note rather than every character, so some characters between emphasized lyrics may go unshown. Each lyric shows every TextAlive character whose start time falls in its epsilon-adjusted hold window (typically the 1–4 syllables it covers), unless the chart provides an explicit character override. A sung character's onset commonly leads its note, so the window starts 100 ms before the note time to admit a leading character. It ends 100 ms before the hold end, so a character within 100 ms of the bounding event (the next note or `end` marker) is excluded and belongs to the next boundary — unless the lyric is flagged `endchar` (osu's `finish` hitsound on the clap), which instead extends the end bound to 100 ms *past* the hold end to claim that closing syllable. Adjacent lyric windows therefore partition the syllables with no overlap. When no character onsets near the note time (nothing in the ±100 ms around it), the syllable it sits on began earlier and is still in progress, so that in-progress character is prepended — but only from this lyric's side of the partition (the gap after the previous note), so a long syllable the previous lyric already showed is not repeated. If the window is still empty — the syllable leads its note by more than 100 ms, so no onset landed inside — the note falls back to the character immediately before the window, taken only from the gap after the previous note so it never borrows a previous lyric's character. If even that finds nothing, the note shows nothing and a warning is logged before play.
