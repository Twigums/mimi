...
Lyric notes display the Japanese characters sung during the hold: every TextAlive character whose start time falls in the note's character window (so one note shows the 1–4 syllables it covers), unless the chart provides an explicit character override. A character's sung onset can lead its lyric note by a few tens of milliseconds, so the window does not begin at the note time — it begins where the previous note ended (minus a 30 ms tolerance), tiling the timeline so a leading character, even one falling in the gap left by an intervening flow or cut note, is still attached to the lyric it belongs to instead of being dropped. The window ends 30 ms before the hold end, so a character within 30 ms of the bounding event (the next note or `end` marker) is excluded and belongs to the next boundary. Adjacent lyric windows therefore partition the syllables with no overlap and no gaps. If no vocal character falls in the window, the note shows nothing and a warning is logged before play.
