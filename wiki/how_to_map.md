...
The difficulty selector also computes note count, cut/flow/lyric breakdown, playable length, and note density from the note rows. `ar` remains chart metadata and is not shown in the home difficulty detail.
...
| `break` | A standalone line (not a note) that ends the current flow phrase and starts the next anchor on a fresh combo. Place it before a note when that note or the immediately preceding note is `flow` and the phrase should break (mirrors osu! new combo). The following row compiles with `newCombo: true`. When there is **no** `break` on a side, the first/last flow anchor of a phrase may use the adjacent non-flow object's position for auto tangent heading only — ribbons still link flow anchors alone |
...
| `text=<substring>` | Lyric text without commas | `m` only. Binds the move to the phrase that contains this exact character substring (kanji included), disambiguating overlapping lead/chorus lines. Omit on non-overlapping phrases — time containment is enough. |
