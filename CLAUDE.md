...
- `src/ts/react/` — React components:
  ...
  - `ResultsOverlay.tsx` — post-song results screen (grade, stats, share, try again). Every non-Tier-3 hit (GREAT/GOOD/MISS) is broken down along **three linked dimensions**: the tier row (`results-breakdown`), and — under the Issues label — a note-kind row (cut/flow/lyric) above the issue row (`IssueReason`: `timing`/`contact`/`direction`/`gesture`, the binding constraint from `judgement.ts` that held the hit below Tier 3). Hovering any cell scopes the *other two* dimensions to hits matching it (`countWhere` over the flattened `npHits`) and re-highlights its own row; the hovered/matching cells glow (`is-active`) and everything else dims (`is-dim`). The Issues label shows a `· <filter>` indicator while a tier or note-kind cell is hovered (an issue hover scopes the tier/note rows, not the issue row, so it shows none)
