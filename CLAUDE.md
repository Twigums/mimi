...
- `src/ts/game/` — Rhythm game engine island:
  ...
  - `judgement.ts` — Pure gesture judgement (`judgeGesture`): selects the best sub-gesture in the metric window and returns `noGesture`/`pending`/`judged`. Timing/gesture tiers and `IssueReason` live here. `canStillImprove` holds a pending note until a better timing tier becomes impossible; to keep non-perfect feedback from lagging behind an **early** cut (issue #53), `gestureSettled` finalizes a cut hit early once the pointer has left the contact zone (`> CUT_CONTACT_TIER1`), gated by `prevNoteTime` so the earliest commit is `min(note.time − TIER3_MS, prevNoteTime)` — a sweep toward an adjacent note can't claim this note during the previous note's territory, and the `min` clamps to the note's own perfect-window start when a near-simultaneous previous note would otherwise over-delay it. `engine.ts` passes `notes[i−1].time` as `prevNoteTime`
