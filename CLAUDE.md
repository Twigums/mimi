...
- `src/tools/osu2mimi.ts` — CLI converter from `.osu` format to `.mimi`: each non-spinner/non-hold hit object (hitcircle or slider head) becomes a flow anchor (`s`) by default, or a lyric note (`l`) if it carries a clap hitsound (`OSU_CLAP`); directions are no longer derived (flow takes the runtime ribbon tangent), so every row emits `degrees` `0`
...
- `degrees`: direction in screen coordinates (0 = right, 90 = down, -90 = up); converted to runtime radians on compile. Used only by cut notes — flow anchors override it at load time with the ribbon tangent derived from neighbouring anchors, and lyric notes ignore it
