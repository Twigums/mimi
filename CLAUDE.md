...
- `src/ts/react/` — React components:
  - `GameSurface.tsx` — canvas + score display + hit feedback toasts + `ResultsOverlay` + the `.game-frame` cloud-border SVG (circle set rebuilt from the game area's measured size via ResizeObserver)
