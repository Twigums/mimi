...
| `src/ChartCompiler.hs` | `chartCompiler` — compiles `.mimi` chart files into `Note[]` JSON; an `auto` (or empty) `degrees` field compiles with no `directionPinned`, a numeric value emits `"directionPinned": true` |
