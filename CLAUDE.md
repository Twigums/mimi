...
| `src/ChartCompiler.hs` | `chartCompiler` — compiles `.mimi` chart files into `Note[]` JSON; an empty `degrees` field compiles with no `directionPinned`, a present value emits `"directionPinned": true` |
...
- `src/tools/osu2mimi.ts` — CLI converter from `.osu` format to `.mimi`: each non-spinner/non-hold hit object (hitcircle or slider head) becomes a flow anchor (`s`) by default, or a lyric note (`l`) if it carries a clap hitsound (`OSU_CLAP`); directions are not authored (flow takes the runtime ribbon tangent), so every row emits an **empty** `degrees` field
...
- `degrees`: direction in screen coordinates (0 = right, 90 = down, -90 = up); converted to runtime radians on compile. Cut notes always use it; lyric notes ignore it. For a flow anchor it is optional — an **empty** field (`s, t, , x, y`) derives direction from the ribbon tangent (compiler sets no `directionPinned`), while a value **pins** that anchor's tangent (compiler emits `"directionPinned": true`)
