...
- `src/ts/react/` — React components:
  ...
  - `ResultsOverlay.tsx` — post-song results screen, a two-column panel (collapses to one column under 560px): a left hero (grade with a radial halo + stamp-in, count-up accuracy/score, max combo, the accuracy distance to the next grade up via `GRADE_STEPS`, and a Full Combo / All Perfect badge) and a right analytics column. The song name + artist are folded into the panel head and the background `.game-song-info` is hidden while results show (so nothing sharp bleeds through the veil, which carries no `backdrop-filter` — a nested one promoted a composited layer that `.game-area`'s fractional translate resampled, softening the text; see `blur-repro.html`). A full-width `TimingHistogram` bins every accepted hit's `offsetMs` (negative = early, positive = late) into an SVG strip centred on 0 with a mean marker and a one-line coaching verdict ("Tends early Nms" / "On time"). Enter retries, Escape returns. Count-up and the staggered rise/stamp-in are gated behind `prefers-reduced-motion`. Every non-Tier-3 hit (GREAT/GOOD/MISS) is broken down along **three linked dimensions**: the tier row (`results-breakdown`), and — under the Issues label — a note-kind row (cut/flow/lyric) above the issue row (`IssueReason`: `timing`/`contact`/`direction`/`gesture`, the binding constraint from `judgement.ts` that held the hit below Tier 3). Hovering any cell scopes the *other two* dimensions to hits matching it (`countWhere` over the flattened `npHits`) and re-highlights its own row; the hovered/matching cells glow (`is-active`) and everything else dims (`is-dim`). The Issues label shows a `· <filter>` indicator while a tier or note-kind cell is hovered (an issue hover scopes the tier/note rows, not the issue row, so it shows none), or a quiet `· hover to filter` hint when nothing is focused
