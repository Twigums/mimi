...
- `src/ts/react/` — React components:
  ...
  - `ResultsOverlay.tsx` — post-song results screen, a two-column panel (collapses to one column under 560px): a left hero (grade with a radial halo + stamp-in, count-up accuracy/score, max combo, the accuracy distance to the next grade up via `GRADE_STEPS`, and a Full Combo / All Perfect badge) and a right analytics column. Takes `difficulty` (from the `?d=` URL param), `level` (looked up from the `data-song-levels` id→level map by the active difficulty), and `bpm` (from `data-song-bpm`) props — all injected by `site.hs` — for the chart-data section, where the difficulty shows as a badge ("HARD lv. 4"). The song name + artist are folded into the panel head and the background `.game-song-info` is hidden while results show (so nothing sharp bleeds through the veil, which carries no `backdrop-filter` — a nested one promoted a composited layer that `.game-area`'s fractional translate resampled, softening the text; see `blur-repro.html`). A full-width `TimingHistogram` bins accepted hits' `offsetMs` (negative = early, positive = late) into an SVG strip whose bounds are **constant**, pinned to the GOOD window (`±TIER1_MS`) so the scale never shifts between runs; it always covers the whole run (hover filtering scopes the breakdown rows, not the timing distribution) and carries a mean marker, a one-line tendency verdict ("Tends early" / "Tends late", or none when on time), and the precise average offset. Enter retries, Escape returns. Count-up and the staggered rise/stamp-in are gated behind `prefers-reduced-motion`. The right column has **three linked cross-filter dimensions**: the tier row (`results-breakdown`, PERFECT/GREAT/GOOD/MISS), a chart-data section (`results-chart`: difficulty/bpm meta + a `results-notes` row showing each kind's **total chart note count** from `stats.noteCounts`, always cut/flow/lyric even at 0), and the Issues row (`results-issues__list`, all four `IssueReason`s — `timing`/`contact`/`direction`/`gesture`, the binding constraint from `judgement.ts` — always shown even at 0, each a colour-accented chip). Note + issue chips sit in fixed-column grids so counts never reflow. Hovering a note-kind or tier cell scopes the dynamic dimensions (issue/tier counts via `countWhere` over the flattened `npHits`) to matching hits; the note row's counts stay the fixed chart totals but still act as a filter. Hovered/matching cells glow (`is-active`), the rest dim (`is-dim`) — and any zero-count cell stays dimmed even at rest. The Issues label trails a `· <filter>` indicator naming the active scope (tier or note name), or — on a **timing**-issue hover — the early/late split of timing errors only (`issue === "timing"`, which excludes PERFECT), or a quiet `· hover to filter` hint when idle. The panel is themed via panel-scoped `--rp-*` tokens (light cloud surface with a soft blue gradient + deeper legible accents by day; deep-navy night surface with the bright pastel accents under `html.theme-dark`)
