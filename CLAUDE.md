...
The `song-chart` path, `textalive-token`, and `song-bpm` (read from the first available difficulty's `.mimi` header, like the manifest) are injected by `site.hs` (not frontmatter).
